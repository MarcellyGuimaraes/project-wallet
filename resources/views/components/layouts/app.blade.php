            ...
            <div class="max-w-5xl mx-auto px-4 flex items-center justify-between h-16">
                <div class="flex items-center gap-8">
                    ...
                    <div class="hidden sm:flex items-center gap-6 h-16">
                        ...
                        <a href="{{ route('wallet.transfer') }}" class="h-full flex items-center text-sm font-medium tracking-wide border-b-2 transition-colors {{ $navLink('wallet.transfer') }}">Transferir</a>
                        <a href="{{ route('wallet.history') }}" class="h-full flex items-center text-sm font-medium tracking-wide border-b-2 transition-colors {{ $navLink('wallet.history') }}">Histórico</a>
                        @if (auth()->user()->isAdmin())
                            <a href="{{ route('admin.users') }}" class="h-full flex items-center text-sm font-medium tracking-wide border-b-2 transition-colors {{ $navLink('admin.users') }}">Administração</a>
                        @endif
                    </div>
                </div>
