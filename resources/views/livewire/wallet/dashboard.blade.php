<div class="space-y-6">
    <div class="pb-card-hero p-8">
        <div class="relative z-[1] flex items-start justify-between">
            ...
            <span class="text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-700">Wallet</span>
        </div>

        <p class="relative z-[1] text-xs font-medium uppercase tracking-[0.25em] text-platinum-300 mt-6">Saldo disponível</p>
        <p class="relative z-[1] text-4xl sm:text-5xl font-bold mt-2 tabular-nums tracking-tight {{ $wallet->balance < 0 ? 'text-red-600' : 'text-slate-950' }}">
            ...
            @forelse ($recentTransactions as $transaction)
                ...
                <div class="flex items-center justify-between px-6 py-3.5">
                    <div class="flex items-center gap-3">
                        <span class="grid place-items-center size-8 rounded-full ring-1 {{ $signed >= 0 ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-red-500/10 text-red-300 ring-red-500/25' }}">
                    ...
                    <span class="text-sm font-semibold tabular-nums {{ $signed >= 0 ? 'text-emerald-700' : 'text-red-300' }}">
